...
# model_name = 'rand_init_no_train_model_epoch_'+str(num_epochs)+current_time+'.h5'
model_name = 'rand_init_no_train_model_epoch_120200203-105707.h5'
...
def print_num_labs(lab_arr):
    uni_lab = np.unique(lab_arr)
    for lab in uni_lab:
        print("The label index and number are: {}, {}.".format(lab, np.sum(lab_arr==lab)))
...
for i, (image, lab) in enumerate(te_data.data.unbatch().take(num_samples),1):
    ...
    print("The labels of the true:")
    print_num_labs(lab)
    print("\n")
    print("The labels of the predicted:")
    print_num_labs(pred)
    print("\n-------------")
